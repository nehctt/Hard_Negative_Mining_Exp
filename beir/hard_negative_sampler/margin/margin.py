...
for i in tqdm(range(cos_scores.shape[0])):
    ...
    for pos_text in pos_doc_texts:
        random_sample = random.sample(range(cos_scores.shape[1]), 50)
        negative_sample_count = 0
        for j in random_sample:
            if corpus_ids[j] in qrels[queries_ids[i]].keys():
                continue
            if margin > cos_scores[i][j]:
                continue
...
with open(f'../../datasets/nfcorpus-hns/margin_{num_hard_negatives}.jsonl', 'w') as file:
